function [D, alphas] = Leearn_D_and_alphas(Img, patchwidth)
    ...
    D = initDict(Img, delta);
%     indices = randperm(size(img,2), 1000);    
%     [~, D] = kmeans(img(:, indices)', delta);
%     D = normc(D'); %Normalize the Dictionary!
    
    %K = floor(0.03*delta);

    param.L = 3;%K;   % number of elements in each linear combination.
    ...
    [D,~]  = KSVD(X,param);
    alphas_nz = full(oOMP(D,img, param.L));
